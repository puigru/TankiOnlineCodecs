# Codec definitions autogenerated on Fri, 04 Sep 2020 04:21:17 GMT
# Decompiler: JPEXS Free Flash Decompiler v.11.3.0
# game-a51b1d2f303778d92c3a285c8cc1bfa3.swf
# entrance-1107d430f5183cfa7c444bb833d8fd3f.swf
...
class EffectsCC(Codec):
    def read(self, packet, optional):
        data = super().read(packet, optional)
        items = list()
        for _ in range(protocol.decode_length(packet)):
            items.append(EffectItem().read(packet, optional))
        data['effects'] = items
        return data
...
class FlashToHtmlMigrationCC(Codec):
    def read(self, packet, optional):
        data = super().read(packet, optional)
        data['downloadLink'] = packet.readString()
        data['flashDisabled'] = bool(packet.readByte())
        data['flashTemporarilyDisabled'] = bool(packet.readByte())
        data['inactiveTimeSec'] = packet.readInt()
        data['timerTimeSec'] = packet.readInt()
        return data

class FlashToHtmlMigrationModelBase_goToLink(Codec):
    def read(self, packet, optional):
        data = super().read(packet, optional)
        data['link'] = packet.readString()
        return data

class FlashToHtmlMigrationModelServer_requestHtmlLink(Codec):
    pass
...
CODECS = {
    ...
    1002072591534861816: FlashToHtmlMigrationCC(),
    6896835139211680506: FlashToHtmlMigrationModelBase_goToLink(),
    1129381279043996655: FlashToHtmlMigrationModelServer_requestHtmlLink(),
